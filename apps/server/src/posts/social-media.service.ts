...
export class SocialMediaService {
  ...
  async postToLinkedIn(post: any, user: any) {
    try {
      ...
      if (!linkedinAccount?.access_token) {
        throw new Error(
          'No LinkedIn access token found. User needs to authenticate with LinkedIn first.',
        );
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          this.logger.error(
            `LinkedIn authentication error: ${response.status} - ${errorText}`,
          );
          throw new Error(
            'LinkedIn access token has expired. Please re-authenticate with LinkedIn in your account settings.',
          );
        }
  ...
  private async uploadLinkedInImage(
    ...
    try {
      ...
      if (!registerResponse.ok) {
        throw new Error(
          `Failed to register image upload: ${registerResponse.status}`,
        );
      }
      ...
      if (!uploadResponse.ok) {
        throw new Error(
          `Failed to upload image to LinkedIn: ${uploadResponse.status}`,
        );
      }

      this.logger.log(
        `Successfully uploaded image to LinkedIn. Asset: ${asset}`,
      );
      return asset;
    } catch (error) {
  ...
  async refreshTwitterToken(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(
          `Twitter token refresh failed: ${response.status} - ${JSON.stringify(errorObj)}`,
        );
  ...
  async postToTwitter(post: any, user: any) {
    try {
      ...
      if (!twitterAccount?.access_token) {
        throw new Error(
          'No Twitter access token found. User needs to authenticate with Twitter first.',
        );
      }
      ...
      if (twitterAccount.refresh_token) {
        try {
          const tokens = await this.refreshTwitterToken(
          ...
          this.logger.warn(
            `Token refresh failed, trying with existing token: ${refreshError.message}`,
          );
        }
      }
  ...
  async publishPost(postId: string, retryCount = 0) {
    ...
    try {
      ...
      if (
        error.message?.includes('LinkedIn access token has expired') ||
        error.message?.includes('No LinkedIn access token found')
      ) {
        this.logger.error(
          `LinkedIn authentication error - no retry: ${error.message}`,
        );
        throw error;
      }
      ...
      if (retryCount < maxRetries) {
        this.logger.warn(
          `Retrying post ${postId} (attempt ${retryCount + 1}/${maxRetries})`,
        );
      ...
      this.logger.error(
        `Failed to publish post ${postId} after ${maxRetries} attempts`,
      );
      throw error;
    }
  }
